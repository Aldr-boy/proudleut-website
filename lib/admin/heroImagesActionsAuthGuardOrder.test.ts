...
// Strukturelle Regressionspruefung: updateHeroWallSelectionAction in
// app/admin/hero-images/actions.ts muss requireAdminSession() als
// ALLERERSTE Anweisung aufrufen -- identisches Muster wie
// eventTypesActionsAuthGuardOrder.test.ts und die uebrigen
// *ActionsAuthGuardOrder-Tests in diesem Verzeichnis. Bewusst eine
// eigene, kleine Kopie statt einer geteilten Abstraktion (andere
// Quelldatei). Es existiert in diesem Repo keine Server-Action-Mocking-
// Infrastruktur und next/headers.cookies() funktioniert nur innerhalb
// eines echten Next.js-Request-Scopes -- ein echter End-to-End-
// Actiontest ist daher nicht sinnvoll isoliert moeglich. Die reine
// Session-Vergleichslogik selbst ist separat in
// isValidAdminSession.test.ts unit-getestet.
const actionsPath = path.join(
  path.dirname(fileURLToPath(import.meta.url)),
  '..', '..', 'app', 'admin', 'hero-images', 'actions.ts'
)
const source = readFileSync(actionsPath, 'utf8')

const PROTECTED_ACTIONS = ['updateHeroWallSelectionAction']
...
test('updateHeroWallSelectionAction existiert in actions.ts', () => {
...
test('requireAdminSession wird in app/admin/hero-images/actions.ts importiert', () => {
  assert.match(source, /import \{ requireAdminSession \} from ['"]@\/lib\/admin\/requireAdminSession['"]/)
})

test('updateHeroWallSelectionAction ruft ausschliesslich die RPC update_hero_wall_selection auf -- kein direkter Tabellen-Write auf media_assets', () => {
  assert.ok(source.includes("rpc('update_hero_wall_selection'"), "erwarteter RPC-Aufruf rpc('update_hero_wall_selection'...) nicht gefunden")
  assert.ok(!source.includes(".from('media_assets').update("), 'Datei darf hero_wall/hero_wall_position/hero_focus nicht per direktem .update() setzen -- Ziel ist ein reiner RPC-Schreibpfad')
})
