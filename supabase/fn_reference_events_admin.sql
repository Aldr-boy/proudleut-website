...
-- bfyucjjyarvqeftqqihm). Erst nach supabase/reference_events_admin_read_grant.sql
-- ausfuehren (dort erhaelt service_role SELECT auf public.reference_events;
-- diese Datei fuegt keine eigenen direkten Table-Grants hinzu).
--
-- Sprint: Referenzverwaltung im Band-Admin (V1)
-- Datum des Entwurfs: 2026-08-16
--
-- Vier Referenz-Event-spezifische Funktionen statt einer generischen
...
-- genau eine Aufgabe. Alle vier teilen sich dasselbe Sicherheitsmodell.
--
-- Funktionen:
--   public.fn_reference_event_create(
--     p_band_id uuid, p_event_name text, p_location_name text,
--     p_city text, p_year integer
--   ) returns public.reference_events
--
--   public.fn_reference_event_update(
...
--
-- Scope V1 (Auftrag "Referenzverwaltung im Band-Admin (V1)"):
--   Nutzerseitig pflegbar sind ausschliesslich event_name, location_name,
--   city, year. sort_order wird ausschliesslich ueber
--   fn_reference_event_move veraendert, nie ueber ein Formularfeld.
--   event_type_id, description, url, is_featured bleiben von V1
--   vollstaendig unberuehrt -- weder lesend im Formular noch schreibend
--   in create/update. Die UPDATE-Anweisung in
--   fn_reference_event_update enthaelt diese vier Spalten deshalb
--   bewusst NICHT im SET, damit bestehende Werte garantiert erhalten
--   bleiben (bei create bleiben sie auf ihren Tabellen-Defaults:
--   event_type_id/description/url = NULL, is_featured = false).
--
-- Validierung (RAISE EXCEPTION vor jeder Schreiboperation):
...
--   - location_name/city: trim(), leerer String wird zu NULL (identische
--     Normalisierung wie bereits an anderer Stelle im Repo ueblich,
--     kein neues Modell)
...
--     vier Funktionen. Der einzige direkte Table-Grant fuer service_role
...
--     service_role fuer jede der vier Funktionen einzeln: Postgres
...
-- ------------------------------------------------------------
-- fn_reference_event_update -- nur event_name/location_name/city/year
-- + updated_at. id, band_id, sort_order, event_type_id, description,
...
-- ------------------------------------------------------------
-- fn_reference_event_delete -- loescht die Zielzeile und nummeriert die
-- verbleibenden Referenzen dieser Band anschliessend luecken-/
-- duplikatfrei neu.
-- ------------------------------------------------------------
...
-- ------------------------------------------------------------
-- fn_reference_event_move -- normalisieren, dann mit dem tatsaechlichen
-- Nachbarn tauschen (identischer Algorithmus wie
...
-- Rechte -- deshalb alle drei Rollen explizit einzeln je Funktion.
-- ------------------------------------------------------------
