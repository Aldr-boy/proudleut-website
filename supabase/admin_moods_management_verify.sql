...
-- zeigen. Kein INSERT/UPDATE/DELETE, keine temporaeren Tabellen.
-- ============================================================
...
-- PUBLIC ist eine Pseudo-Rolle und kann nicht als Benutzername an
-- has_function_privilege(text, ...) uebergeben werden. Deshalb wird die
-- effektive Funktions-ACL direkt ausgewertet. grantee=0 steht fuer PUBLIC;
-- acldefault deckt auch den Fall proacl IS NULL korrekt ab.
...
band_moods_pk_row as (
  select
    ...
    coalesce((select pg_get_constraintdef(oid) from pg_constraint where conrelid = 'public.band_moods'::regclass and contype = 'p'), '(keine PK gefunden)') as actual,
    coalesce((select pg_get_constraintdef(oid) from pg_constraint where conrelid = 'public.band_moods'::regclass and contype = 'p') = 'PRIMARY KEY (band_id, mood_id)', false) as match
),
band_moods_sort_order_not_null_row as (
  select
    ...
    coalesce((select is_nullable from information_schema.columns where table_schema='public' and table_name='band_moods' and column_name='sort_order'), '(Spalte nicht gefunden)') as actual,
    coalesce((select is_nullable='NO' from information_schema.columns where table_schema='public' and table_name='band_moods' and column_name='sort_order'), false) as match
),
band_moods_sort_order_check_row as (
  select
    ...
    coalesce((select string_agg(pg_get_constraintdef(oid), ' | ') from pg_constraint where conrelid='public.band_moods'::regclass and contype='c'), '(kein CHECK gefunden)') as actual,
    exists (select 1 from pg_constraint where conrelid='public.band_moods'::regclass and contype='c' and pg_get_constraintdef(oid) ~* 'sort_order\s*>=\s*0') as match
),
band_moods_no_new_unique_on_sort_order_row as (
  select
    ...
    coalesce((select string_agg(c.conname, ', ') from pg_constraint c where c.conrelid='public.band_moods'::regclass and c.contype='u' and c.conkey @> (select array_agg(attnum) from pg_attribute where attrelid='public.band_moods'::regclass and attname='sort_order')), '(keine gefunden)') as actual,
    not exists (select 1 from pg_constraint c where c.conrelid='public.band_moods'::regclass and c.contype='u' and c.conkey @> (select array_agg(attnum) from pg_attribute where attrelid='public.band_moods'::regclass and attname='sort_order')) as match
),
moods_slug_not_null_row as (
  select
    ...
    'NOT NULL'::text as expected,
    coalesce((select is_nullable from information_schema.columns where table_schema='public' and table_name='moods' and column_name='slug'), '(Spalte nicht gefunden)') as actual,
    coalesce((select is_nullable='NO' from information_schema.columns where table_schema='public' and table_name='moods' and column_name='slug'), false) as match
),
moods_slug_unique_row as (
  select
    'moods_slug_unique'::text as report_section,
    'moods.slug'::text as key,
    'UNIQUE-Constraint auf slug vorhanden'::text as expected,
    coalesce((select string_agg(c.conname, ', ') from pg_constraint c where c.conrelid='public.moods'::regclass and c.contype='u' and c.conkey @> (select array_agg(attnum) from pg_attribute where attrelid='public.moods'::regclass and attname='slug')), '(keine gefunden)') as actual,
    exists (select 1 from pg_constraint c where c.conrelid='public.moods'::regclass and c.contype='u' and c.conkey @> (select array_agg(attnum) from pg_attribute where attrelid='public.moods'::regclass and attname='slug')) as match
),
moods_slug_format_row as (
  select
    'moods_slug_format'::text as report_section,
    'moods.slug (alle Zeilen)'::text as key,
    'alle Slugs erfuellen ^[a-z0-9-]+$'::text as expected,
...
moods_status_not_null_row as (
  select
    ...
    'NOT NULL'::text as expected,
    coalesce((select is_nullable from information_schema.columns where table_schema='public' and table_name='moods' and column_name='status'), '(Spalte nicht gefunden)') as actual,
    coalesce((select is_nullable='NO' from information_schema.columns where table_schema='public' and table_name='moods' and column_name='status'), false) as match
),
moods_status_values_row as (
  select
    'moods_status_values'::text as report_section,
    'moods.status (alle Zeilen)'::text as key,
    'alle vorhandenen Statuswerte sind active oder archived'::text as expected,
...
union all select * from band_moods_pk_row
union all select * from band_moods_sort_order_not_null_row
union all select * from band_moods_sort_order_check_row
union all select * from band_moods_no_new_unique_on_sort_order_row
union all select * from moods_slug_not_null_row
union all select * from moods_slug_unique_row
union all select * from moods_slug_format_row
union all select * from moods_status_not_null_row
union all select * from moods_status_values_row
