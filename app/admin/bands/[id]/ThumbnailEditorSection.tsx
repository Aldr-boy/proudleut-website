...
import { updateBandThumbnailAction } from './actions'
...
export function ThumbnailEditorSection({
  ...
  bandId: string
  // aktuelles Thumbnail dieser Band, oder null (Empty State)
  thumbnailImage: { url: string; alt: string } | null
  // true, wenn das Laden der media_assets-Zeilen fehlgeschlagen ist -- darf
  // NIE als "kein Thumbnail vorhanden" interpretiert werden (fail-closed).
  loadError?: boolean
  successMsg?: string
  errorMsg?: string
}) {
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-xl p-5 mb-5">
      ...
      <p className="text-xs text-gray-400 mb-4">
        ...
        Hero-Bild -- ein neuer Upload ersetzt nur das Thumbnail, nicht das Hero-Bild.
      </p>
      ...
      <div className="mb-4">
        ...
        {thumbnailImage ? (
          <div className="relative w-full max-w-sm h-40 rounded-lg overflow-hidden border border-gray-200 bg-gray-50">
            <Image src={thumbnailImage.url} alt={thumbnailImage.alt} fill className="object-cover" sizes="384px" />
          </div>
        ) : (
          <div className="w-full max-w-sm h-40 rounded-lg border border-dashed border-gray-300 bg-gray-50 flex items-center justify-center">
            <p className="text-sm text-gray-400">Kein Thumbnail vorhanden</p>
          </div>
        )}
      </div>

      <form action={updateBandThumbnailAction} className="space-y-3">
