  ...
  return (
    <article>
      ...
      >
        {/* ─── Hauptinhalt ─── */}
        <div>
          <h1 className="text-3xl md:text-4xl font-bold text-pl-text leading-tight">
            {band.name}
          </h1>
          {band.category && (
            <p className="text-pl-text-muted mt-1 mb-2">{band.category}</p>
          )}
          {band.shortDescription && (
            <p className="text-pl-text text-lg mb-4">{band.shortDescription}</p>
          )}

          {/* Mobile CTA – nur auf kleinen Screens (lg:hidden) */}
          {websiteUrl && (
            <div className="lg:hidden mb-8">
              <a
                ...
                className="flex items-center justify-center w-full px-6 py-3 rounded-full
                           bg-pl-primary text-white font-semibold
            ...
            <section className="mb-8">
              ...
              <div className="flex flex-wrap gap-2">
                {band.eventTypes.map((et) => {
                  ...
                  const base =
                    'px-3 py-1 rounded-full text-sm border border-white/10 text-pl-text-muted';
                  return catSlug ? (
                    <Link
                      key={et}
                      href={`/veranstaltung/${catSlug}`}
                      className={`${base} hover:border-pl-primary hover:text-pl-text motion-safe:transition-colors`}
                    >
                      {et}
                    </Link>
                  ) : (
          ...
          {hasWeddingInfo && (
            <section className="mb-8">
              ...
              <dl className="grid grid-cols-2 gap-x-6 gap-y-2 text-sm text-pl-text-muted">
                {band.weddingInfo?.bandSize && (
                  <>
                    <dt className="text-pl-text-muted/60">Bandgröße</dt>
                    <dd>{band.weddingInfo.bandSize}</dd>
                  </>
                )}
                {band.weddingInfo?.feeRange && (
                  <>
                    <dt className="text-pl-text-muted/60">Gage</dt>
                    <dd>{band.weddingInfo.feeRange}</dd>
                  </>
                )}
                {band.weddingInfo?.constellation && (
                  <>
                    <dt className="text-pl-text-muted/60">Konstellation</dt>
                    <dd>{band.weddingInfo.constellation}</dd>
                  </>
                )}
                {band.weddingInfo?.moderation != null && (
                  <>
                    <dt className="text-pl-text-muted/60">Moderation</dt>
                    <dd>{band.weddingInfo.moderation ? 'Ja' : 'Nein'}</dd>
                  </>
                )}
                {band.weddingInfo?.kidnappingBride != null && (
                  <>
                    <dt className="text-pl-text-muted/60">Brautentführung</dt>
                    <dd>{band.weddingInfo.kidnappingBride ? 'Ja' : 'Nein'}</dd>
                  </>
                )}
                {band.weddingInfo?.possiblePlaytimes && (
                  <>
                    <dt className="text-pl-text-muted/60">Spielzeiten</dt>
                    <dd>{band.weddingInfo.possiblePlaytimes}</dd>
                  </>
                )}
              </dl>
            </section>
            ...
            <section className="mb-8">
              ...
              <ul className="space-y-1">
                {band.referenceEvents.slice(0, 8).map((ev, i) => (
                  <li key={i} className="text-pl-text-muted text-sm">
                    {ev.eventName}
                    {ev.city && ` · ${ev.city}`}
                    {ev.year && ` (${ev.year})`}
                  </li>
          ...
          {embedUrl && (
            <section className="mb-8">
              ...
              <div className="relative w-full aspect-video rounded-xl overflow-hidden bg-pl-surface">
                <iframe
                  src={embedUrl}
            ...
            <div className="flex gap-3 flex-wrap">
              {band.socialLinks.instagram && (
                <a
                  ...
                  className="text-pl-text-muted hover:text-pl-primary motion-safe:transition-colors"
                >
