...
// Strukturelle Regressionspruefung: jede der fuenf Bild-Actions
// (Hero/Thumbnail/Galerie-Add/Delete/Move) muss requireAdminSession()
// als ALLERERSTE Anweisung aufrufen -- vor jedem FormData-Zugriff, vor
// jedem Service-Role-Client, vor jedem DB-/Storage-/RPC-Zugriff. Es
// existiert in diesem Repo keine Server-Action-Mocking-Infrastruktur und
// next/headers.cookies() funktioniert nur innerhalb eines echten
// Next.js-Request-Scopes -- ein echter End-to-End-Actiontest ist daher
// nicht sinnvoll isoliert moeglich. Dieser Test prueft daher die
// tatsaechliche Quelldatei textuell: ein spaeteres versehentliches
// Verschieben des Guards (z. B. hinter formData.get() oder
// createAdminClient()) wuerde diesen Test zuverlaessig brechen. Die reine
// Session-Vergleichslogik selbst ist separat in
// requireAdminSession.test.ts unit-getestet.
...
test('genau die fuenf erwarteten Bild-Actions existieren in actions.ts', () => {
...
for (const functionName of PROTECTED_ACTIONS) {
  ...
  test(`${functionName}: requireAdminSession() steht vor jedem Service-Role-Client, DB-/Storage-/RPC-Zugriff und jeder Revalidierung`, () => {
    ...
    for (const marker of ['createAdminClient()', '.storage', '.rpc(', "from('media_assets')", 'revalidatePath(']) {
